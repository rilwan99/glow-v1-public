...
impl num_traits::CheckedDiv for Number128 {
    fn checked_div(&self, v: &Self) -> Option<Self> {
        self.0.checked_mul(ONE)?.checked_div(v.0).map(|n| n.into())
    }
}

impl num_traits::CheckedMul for Number128 {
    fn checked_mul(&self, v: &Self) -> Option<Self> {
        Some(Self(self.0.checked_mul(v.0)?.div(ONE)))
    }
}
...
#[cfg(test)]
mod tests {
    use crate::traits::{SafeDiv, SafeMul, SafeSub};
    ...
    #[test]
    fn checked_div() {
        let a = Number128::from_decimal(1_000_000_000i64, -9);
        let b = Number128::from_decimal(1_000_000_000i64, -9);
        let result = a.safe_div(b).unwrap();

        assert_eq!(result, Number128::ONE);

        assert!(Number128::ONE.safe_div(Number128::ZERO).is_err());
        assert_eq!(
            Number128::ZERO.safe_div(Number128::ONE).unwrap(),
            Number128::ZERO
        );
    }
    ...
    #[test]
    fn checked_mul() {
        let a = Number128::from_decimal(1_000_000_000i32, -8);
        let b = Number128::from_decimal(10_000_000_000i64, -8);
        let result = a.safe_mul(b).unwrap();

        assert_eq!(result, Number128::ONE * 1000);

        assert_eq!(
            Number128::ONE.safe_mul(Number128::ZERO).unwrap(),
            Number128::ZERO
        );
    }
    ...
    fn checked_sub() {
        let a = Number128::from_decimal(1_000_000_000i32, -8);
        let b = Number128::from_decimal(1_000_000_000i32, -8);
        let result = a.safe_sub(b).unwrap();

        assert_eq!(result, Number128::ZERO);

        assert_eq!(
            Number128::ONE.safe_sub(Number128::ZERO).unwrap(),
            Number128::ONE
        );
        assert_eq!(
            Number128::ONE.safe_sub(Number128::ONE).unwrap(),
            Number128::ZERO
        );
    }
}
